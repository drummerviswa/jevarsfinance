...
const DepositSpecificLoan = ({ updated, setUpdated }) => {
  ...
  useEffect(() => {
    fetch(`http://15.206.73.76:8800/api/deposit/loans/${customer[0]["Cus_ID"]}`, {
      method: "GET",
    })
  ...
  const handleDelete = async (item) => {
    try {
      await axios.delete(`http://15.206.73.76:8800/api/deposit/loans/${item.Loan_No}`);
  ...
  const handleStatus = async (item) => {
    try {
      await axios.put(
        `http://15.206.73.76:8800/api/deposit/loans/status/${item.Loan_No}`,
        { status: "Closed" }
      );
      setLoans(loans.filter((i) => i.Loan_No !== item.Loan_No));
      setUpdated(!updated);
