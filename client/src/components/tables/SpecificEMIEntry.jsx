...
import EMIEntryModal from "../modals/EMIEntryModal";
import axios from "axios";
import { useSelector } from "react-redux";

function EMISpecificEntry({ updated, setUpdated }) {
  ...
  useEffect(() => {
    fetch(`https://app-1odw.onrender.com/api/emi/entries/${customer[0]["Cus_ID"]}`, {
      method: "GET",
    })
  ...
  const handleDelete = async (item) => {
    try {
      await axios.delete(`https://app-1odw.onrender.com/api/emi/entries/${item.Entry_ID}`);
        ...
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            ...
            <th className="px-6 py-3">
              <div
                className="flex items-center cursor-pointer"
                onClick={() => sortByColumn("Entry_Type")}
              >
                Entry Type
...
export default EMISpecificEntry;
