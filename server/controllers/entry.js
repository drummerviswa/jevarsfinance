...
export const getEntries = (req, res) => {
  const q =
    "SELECT FirstName,LastName,l.*,e.* from entries e INNER JOIN loans l ON e.Loan_No=l.Loan_No INNER JOIN customers ON l.Cus_ID=customers.Cus_ID";
  db.query(q, (err, data) => {
    if (err) return res.status(500).json(err);
    return res.status(200).json(data);
  });
};
export const getEntryFromCustomer = (req, res) => {
  const q =
    "SELECT FirstName,LastName,l.*,e.* from entries e INNER JOIN loans l ON e.Loan_No=l.Loan_No INNER JOIN customers ON l.Cus_ID=customers.Cus_ID WHERE customers.Cus_ID=?";
