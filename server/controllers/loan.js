...
export const updateLoan = (req, res) => {
  const q =
    "UPDATE `loans` SET `LoanType`=?,`Amount`=?,`Interest`=?,`DOB`=?,`Document`=?,`Status`=? WHERE `Loan_No`=?";
  const value = [
    req.body.loanType,
    req.body.amount,
    req.body.interest,
...
export const addLoan = (req, res) => {
  const q =
    "INSERT INTO `loans`( `Loan_No`,`Cus_ID`,`LoanType`, `Amount`, `Interest`, `DOB`, `Document`, `Status`) VALUES (?,?,?,?,?,?,?,?)";
  const value = [
    req.body.Loan_No,
    req.body.Cus_ID,
    req.body.loanType,
